...
<Card key={`${r.district}-${idx}`} className="border-2"> <CardContent className="p-3"> <div className="flex items-center justify-between"> <div className="font-medium">{r.district}</div> <div className="text-sm text-muted-foreground">
{Intl.NumberFormat("en-IN").format(r.persondays)} persondays </div> </div> <div className="text-xs text-muted-foreground">
Households: {Intl.NumberFormat("en-IN").format(r.householdsWorked)} </div> </CardContent> </Card>
))}
{!uniqueRows.length && ( <div className="text-sm text-muted-foreground">
No comparison data available. </div>
)} </div>
)
}
